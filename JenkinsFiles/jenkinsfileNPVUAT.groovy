...
pipeline {
    ...
    parameters {
        ...
        string(name: 'Tags', defaultValue: '@UATNPV', description: 'Select Tags to run, eg: @HC')
        string(name:'EmailID',defaultValue: 'OrbitTestTeam@domesticandgeneral.com',description: 'Send Test Results in Email')
    }
    stages {
        stage('Checkout') {
            steps {
        ...
        stage('Re-Run Failed Test') {
            steps {
                echo 'Executing Failed Scenario\'s'
//                bat 'mvn test -Dtest=RerunTestRunner -DENV=%EnvironmentToExecute% -Dbroswer=%Browser% -DbrowserState=%BrowserState% -Ddataproviderthreadcount=4'

            }
        }
        stage('Generate Report') {
            steps {
                cucumber expandAllSteps: false, failedFeaturesNumber: -1, failedScenariosNumber: -1, failedStepsNumber: -1, fileIncludePattern: 'cucumber.json', hideEmptyHooks: true, jsonReportDirectory: 'target', pendingStepsNumber: -1, skippedStepsNumber: -1, sortingMethod: 'ALPHABETICAL', undefinedStepsNumber: -1
