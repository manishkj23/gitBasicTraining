...
pipeline {
    ...
    parameters {
        ...
        string(name: 'buildStatusColor', defaultValue: '')


        choice(name: 'EnvironmentToExecute', choices: ['prelive','sit','prod'],description: 'Select Environment to Execute')
        ...
        string(name: 'Tags', defaultValue: '@UAT', description: 'Select Tags to run')

        booleanParam(defaultValue: false, description: 'Select / Check if sample tag to be tested - bypass Tags parameter', name: 'sampleRun')
        string(name:'EmailID',defaultValue: 'veera.vantipalli@domesticandgeneral.com',description: 'email to send the report')
    ...
    stages {
        ...
        stage('Test Execution') {
            steps {
//                if("true"=='%sampleRun%') {
//                    bat 'mvn clean test -Dcucumber.options="--tags @SC016" -DENV=%EnvironmentToExecute% -Dbroswer=%Browser% -DbrowserState=%BrowserState% -DenableScreenshotWhenHighlighted=%EnableScreenshots% -DenableVideoRecording=%EnableRecording%'
//                    echo 'Executing Tests - Sample Tests'
//                } else {
                script {
                    try {
                        ...
                        dir("orbTestit/target/cucumber-reports/cucumber-html-reports/"){
                            slackUploadFile channel: '#regression-tests',
                                    credentialId: 'Slack_ID',
                                    filePath: 'overview-features.html	',
                                    initialComment: '${currentBuild.getFullDisplayName()} - ${currentBuild.result} - (<${currentBuild.absoluteUrl}|Open>)'
                        }
                    }
                }
